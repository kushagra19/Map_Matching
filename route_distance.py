...
def shortest_path(G, orig_yx, dest_yx, orig_edge=None, dest_edge=None):
    ...
    if orig_edge == dest_edge:        
        ...
        edge_geo = get_edge_geometry(G, orig_edge)
    ...
    # routing across multiple edges
        ...
        # nx_route = nx_shortest_path(G, orig_edge[int(orig_clip+0.5)],
        #                             dest_edge[int(dest_clip+0.5)], 'length')

        if G.edges[orig_edge]['oneway'] == True:
            # print(int(dest_clip+0.5))
            nx_route = nx_shortest_path(G, orig_edge[1], dest_edge[0], 'length')
        ...
        if len(nx_route) >= 3:
            nx_route_tmp = nx_route
            ### resolve origin
            node1 = nx_route_tmp[0]
            node2 = nx_route_tmp[1]
            # check overlap with first route edge
            route_orig_edge = get_edge_geometry(G, (node1, node2, 0))
            if route_orig_edge.intersects(orig_partial_edge_1) and route_orig_edge.intersects(orig_partial_edge_2):
                node1 = nx_route_tmp[1]
                node2 = nx_route_tmp[2]
                nx_route = nx_route[1:]
